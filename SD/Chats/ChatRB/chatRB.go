...
func main() {
	...
	if len(os.Args) < 2 {
		...
		fmt.Println("go run chatRB.go 0 127.0.0.1:5001  127.0.0.1:6001   127.0.0.1:7001")
		fmt.Println("go run chatRB.go 1 127.0.0.1:5001  127.0.0.1:6001   127.0.0.1:7001")
		fmt.Println("go run chatRB.go 2 127.0.0.1:5001  127.0.0.1:6001   127.0.0.1:7001")
		return
	}
	...
	rb := NewRB(myId, addresses, true)

	//rb.Init(addresses[0])
	//rb.InitD(myId, addresses, true)
	...
	go func() {
		...
		for {
			...
			req := RB_Req_Message{
				Message: msg}
			rb.Req <- req // ENVIA PARA TODOS PROCESSOS ENDERECADOS NO INICIO
		}
	}()
	...
	go func() {
		for {
			in := <-rb.Ind // RECEBE MENSAGEM DE QUALQUER PROCESSO
